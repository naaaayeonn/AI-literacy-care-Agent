...
## ⚖️ Data Attributions & Academic Reference

본 프로젝트(2번 역할)의 RAG 용어풀이 및 가독성/난이도 분석 엔진은 아래의 공공 데이터와 학술 연구를 준수하며, 원 저작자의 권리와 라이선스를 따릅니다.

### 1. 원천 공공 데이터 출처
* **국어 기초 어휘 등급 데이터**: 국립국어원 (2024) `<국어 기초 어휘 선정 및 어휘 등급화 목록>` (공공누리 KOGL 제1유형 적용)
* **표준 전문 용어 데이터**: 문화체육관광부 고시 / 국립국어원 `<국어기본법에 따른 고시 표준 전문 용어>`
* **RAG 기본 용어 사전 데이터**: 국립국어원 `<표준국어대사전>` API 추출본

### 2. 학술적 공식 및 인용 Reference
* **한국어 이독성 모델 설계**: <고승연. (2025). 제2언어 학습자를 위한 한국어 읽기 텍스트 이독성 공식 개발 및 수준 환산. 새국어교육, 143, 245-272.>
  * *비고*: 영어 환경 전용 가독성 수식(Flesch-Kincaid, Gunning Fog, SMOG 등)은 한국어의 교착어 구조(조사/어미 분리)와 어긋나므로 실제 코드 연산에서 전면 배제하고, 한국어 가소성 피처를 응용한 독자적인 가중치 감점 모델을 수립하여 기여하였습니다.
