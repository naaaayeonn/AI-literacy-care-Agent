...
| Milestone | 날짜 | 목표 | 완료 기준 |
|---|---|---|---|
| **M0** | 6/22 (월) | 더미 데이터로 전체 화면 표시 | 점수·본문·집중도가 화면에 보임 |
| **M1** | 6/29 (월) | 폐루프 데모 완성 | 읽기→집중도→넛지→점수 흐름이 화면에서 동작 |
| **M2** | 7/6 (월) | 전 기능 통합 | 백엔드 API 실제 연동, 대시보드 완성 |
| **M3** | 7/10 (금) | 기능 동결 | 버그 수정만, 구조 변경 없음 |
| **M4** | 7/14 (화) | 제출본 점검 | 시연 흐름 반복 실행 가능 |
...
| 날짜 | 작업 | 완료 기준 |
|---|---|---|
| **6/30 (화)** 선작업 | **성장 대시보드(GrowthDashboard)** — 주간/월간 통계 레이아웃 | 대시보드 페이지 UI 완성 |
| **7/1 (수)** 선작업 | **게이미피케이션 UI** — LevelBar·BadgeShelf·XpCounter 실구현 | FloatingPanel + Dashboard에 배지·레벨 표시 |
| | **FloatingControlPanel 완성** — 전체 실시간 지표 연결 | 모든 지표 갱신됨 |
| **7/2~4 (목~토)** 최소 | UI 에셋·아이콘·스타일 정리, 반응형 레이아웃 | 모바일/태블릿 레이아웃 이상 없음 |
| **7/5 (일)** 보강 | 대시보드·게이미피케이션 마무리, 엣지 케이스 정리 | |
| **7/6 (월)** M2 | **실제 백엔드 API 연결** — api.ts fetch 구현, WebSocket 클라이언트 연결 | 더미 데이터 없이 실제 서버 데이터 표시 |

### W3 — 통합·QA (7/7~7/10)

| 날짜 | 작업 | 완료 기준 |
|---|---|---|
| **7/7 (화)** | 통합 연결·UX 다듬기 (③번 API 엔드포인트 최종 연결) | |
| **7/8~9 (수·목)** | UI 버그 수정, 크로스 브라우저 점검 | |
| **7/10 (금)** M3 | **기능 동결** — 이후 폴리시만 | 시연 시나리오 3회 연속 동작 |
...
- [ ] `/reading`, `/dashboard` URL 라우팅 동작
- [ ] RootLayout 헤더 공통 레이아웃
- [ ] ReadingPage 와이어프레임 수준 레이아웃
- [ ] DashboardPage 레이아웃 뼈대
- [ ] API 응답 JSON TypeScript 인터페이스 정의
- [ ] Shared State 타입 보강

### 6/21 Should Have
- [ ] 네비게이션 링크 (읽기 화면 ↔ 대시보드 이동)
- [ ] 페이지 제목 (브라우저 탭 title) 설정
